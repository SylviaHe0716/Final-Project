...
bool solvePuzzle(int N, int **grid, int colorfrom, int colorto, int *S,int thread_id) {
        ...
        if (abs(endNode->row,startNode->row) + abs(endNode->column,startNode->column) > 0) {
            ...
            for (int i = 0; i < directions.size(); i++) {
                if (directions[i] =="connect") {
                    ...
                    printf("connect\n");
                    if (solvePuzzle(N, grid, colorfrom, colorto, S, thread_id)) {
                        return true;
              	    } else {
                        //backtrack...
            ...
            Solutions[color].push_back(S[color]);
            //printf("Solutions\n");
...
#pragma omp for
        for (int i = red; i <= orange; i++) {
            //s = 0;
            if (solvePuzzle(N, array, i, i, S, thread_id)) {
